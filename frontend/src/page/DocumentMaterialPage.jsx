                ...
                <tbody>
                  {project.SOWFile.map((file, idx) => (
                    <tr key={file._id} className="hover:bg-gray-50 transition-colors border-b border-gray-200">
                      ...
                      <td className="px-4 py-3 truncate" title={getDisplayFileName(file.fileName)}>
                      ...
                      <td className="px-4 py-3 text-gray-600">
                        {file.uploadedByName || file.uploadedBy?.name || "Unknown"}
                      </td>
                      <td className="px-4 py-3 flex gap-2">
                        <button
                          className="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition"
                          // onClick={() => {
                          //   const fileUrl = file.url || `${file.fileName}`; // adjust path accordingly
                          //   window.open(fileUrl, "_blank");
                          // }}
                        onClick={() => window.open(file.fileName, "_blank", "noopener,noreferrer")}
                        >
                          View
                        </button>
                        {/* <a
